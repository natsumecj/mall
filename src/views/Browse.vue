<template>
  <div>
    ...
    <van-row>
      <div class="goods" v-for="(item, index) in browses" :key="item.cid">
        <van-col span="7">
          <div class="img">
        ...
        <van-col span="3" align="center" class="close">
          <div @click="del(index)"><van-icon name="close" size="20px" /></div>
        </van-col>
      </div>
    </van-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    del(index) {
      ...
      //     console.log(err);
      // })
      let arr = JSON.parse(localStorage.goods);
      arr.splice(index, 1);
      this.browses = arr
      localStorage.setItem("goods", JSON.stringify(arr));
      // JSON.stringify(localStorage.setItem("goods",this.browses))
    }
  },
  mounted() {
    if (localStorage.goods) {
      this.browses = JSON.parse(localStorage.getItem("goods"));
    }
  },
  created() {},
  filters: {},
